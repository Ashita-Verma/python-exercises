...
def round_scores(student_scores):
    ...
    while student_scores:
        new_list_of_scores.append(round(student_scores.pop()))
...
def letter_grades(highest):
    ...
    threshold_jump = round((highest - 40)/4 )
    for score in range(41 , highest, threshold_jump):
...
def student_ranking(student_scores, student_names):
    ...
    for rank, (name,score) in enumerate(zip(student_names,student_scores), start=1):
        new_list.append(f"{rank}. {name}: {score}")
