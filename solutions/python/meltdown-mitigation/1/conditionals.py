...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    efficiency = int(((voltage * current)/theoretical_max_power) * 100)
    if efficiency >= 80: 
        band = 'green'
    elif efficiency >= 60: 
        band = 'orange'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    reactor_temp = temperature * neutrons_produced_per_second
    if reactor_temp < (threshold * 0.9):
        status = 'LOW'
    elif reactor_temp < (threshold * 1.1):
        status = 'NORMAL'
    else:
        status = 'DANGER'
